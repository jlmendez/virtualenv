...
def run():
        ...
        Q = '''select * from polls_ise1_infra
               where  position ('f-' in mseed_text) =0  
                  and fecha_recepcion > (select max(fecha_recepcion) 
                                     from polls_ise1_infra) - interval '180 seconds'
    ...
    for i in range(1,len(infrasonido_1)-1):
        ...
        if  (abs(c-a) < lim_in[1] or abs(c-a) > lim_su[1]) and (abs(c-p) < lim_in[1] or abs(c-p) > lim_su[1]) and  abs(dif_pa) <0.02:
            ...
            Q= ''' update polls_ise1_infra set infrasonido_1 = '{0}', mseed_text = 'f-1,'||mseed_text  where fecha_sistema = '{1}'
        ...
        if  (abs(c-a) < lim_in[2] or abs(c-a) > lim_su[2]) and (abs(c-p) < lim_in[2] or abs(c-p) > lim_su[2]) and  abs(dif_pa) <0.02:
            ...
            Q= ''' update polls_ise1_infra set infrasonido_2 = '{0}', mseed_text = 'f-2,'||mseed_text where fecha_sistema = '{1}'
        ...
        if  (abs(c-a) < lim_in[3] or abs(c-a) > lim_su[3]) and (abs(c-p) < lim_in[3] or abs(c-p) > lim_su[3]) and  abs(dif_pa) <0.02:
            ...
            Q= ''' update polls_ise1_infra set infrasonido_3 = '{0}', mseed_text = 'f-3,'||mseed_text  where fecha_sistema = '{1}'
        ...
        if  (abs(c-a) < lim_in[4] or abs(c-a) > lim_su[4]) and (abs(c-p) < lim_in[4] or abs(c-p) > lim_su[4]) and  abs(dif_pa) <0.02:
            ...
            Q= ''' update polls_ise1_infra set infrasonido_4 = '{0}', mseed_text = 'f-4,'||mseed_text  where fecha_sistema = '{1}'
            '''.format(infrasonido_4[i],fecha_sistema[i])
            print(Q)
            cursor.execute(Q)

        Q= ''' update polls_ise1_infra set mseed_text = 'f-,'||mseed_text  where fecha_sistema = '{0}'
